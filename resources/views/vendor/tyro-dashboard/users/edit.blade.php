...
<div class="form-group mb-4">
        ...
        <div>
                ...
                <div class="card-body">
                    <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem;">
                        <div class="user-cell-avatar"
                            style="width: 64px; height: 64px; font-size: 1.5rem; {{ (method_exists($editUser, 'hasProfilePhotoColumn') && $editUser->hasProfilePhotoColumn() && $editUser->profile_photo_path) || (method_exists($editUser, 'hasGravatarColumn') && $editUser->hasGravatarColumn() && $editUser->use_gravatar) ? 'background: none; padding: 0;' : '' }}">
                            @if (
                                (method_exists($editUser, 'hasProfilePhotoColumn') &&
                                    $editUser->hasProfilePhotoColumn() &&
                                    $editUser->profile_photo_path) ||
                                    (method_exists($editUser, 'hasGravatarColumn') &&
                                        $editUser->hasGravatarColumn() &&
                                        $editUser->use_gravatar &&
                                        $editUser->email))
                                <img src="{{ $editUser->profile_photo_url }}" alt="{{ $editUser->name }}"
                        ...
                        <div>
                            ...
                            <div style="font-size: 0.875rem; color: var(--muted-foreground);">Member since
                                {{ $editUser->created_at->format('M d, Y') }}</div>
                            @if (method_exists($editUser, 'hasProfilePhotoColumn') &&
                                    $editUser->hasProfilePhotoColumn() &&
                                    $editUser->profile_photo_path)
                                <div style="margin-top: 0.5rem;">
                                    <form id="delete-user-photo-form"
                                        action="{{ route($dashboardRoute::name('users.photo.delete'), $editUser->id) }}"
                                        method="POST" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-sm btn-ghost"
                                            style="color: var(--danger); padding: 0.25rem 0.5rem;"
                                            onclick="showDanger('Remove Photo', 'Are you sure you want to remove this user\'s profile photo?').then(confirmed => { if(confirmed) document.getElementById('delete-user-photo-form').submit(); })">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                stroke-width="2" style="width: 14px; height: 14px; margin-right: 4px;">
                    ...
                    <div style="padding: 1rem; background-color: var(--muted); border-radius: 0.5rem;">
                        ...
                        @if (method_exists($editUser, 'isSuspended') &&
                                $editUser->isSuspended() &&
                                method_exists($editUser, 'getSuspensionReason') &&
                                $editUser->getSuspensionReason())
                            <div style="margin-top: 0.75rem; padding-top: 0.75rem; border-top: 1px solid var(--border);">
                                ...
                                <p style="font-size: 0.875rem; color: var(--muted-foreground); margin-top: 0.25rem;">
                                    {{ $editUser->getSuspensionReason() }}</p>
                            </div>
                        @endif
                    </div>
                    ...
                    @if (config('tyro-login.two_factor.enabled'))
                        <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--border);">
                            <div
                                ...
                                <span style="font-size: 0.875rem; color: var(--muted-foreground);">Two-Factor
                                    Authentication</span>
                            ...
                            @if ($editUser->two_factor_secret)
                                <form action="{{ route($dashboardRoute::name('users.2fa.reset'), $editUser->id) }}"
                                    method="POST" id="reset-2fa-form">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="btn btn-sm btn-outline-warning" style="width: 100%;"
                                        onclick="event.preventDefault(); showConfirm('Reset 2FA', 'Are you sure you want to reset 2FA for this user?').then(confirmed => { if(confirmed) document.getElementById('reset-2fa-form').submit(); })">
                                        Reset 2FA
                                    </button>
                                </form>
                            @endif
                    ...
                    <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--border);">
                        @if (method_exists($editUser, 'isSuspended') && $editUser->isSuspended())
                            <form action="{{ route($dashboardRoute::name('users.unsuspend'), $editUser->id) }}"
                                method="POST">
                                @csrf
            ...
            @if ($editUser->id !== $user->id)
                <div class="card" style="border-color: var(--destructive);">
                    <div class="card-header"
                        style="background-color: color-mix(in srgb, var(--destructive), transparent 90%);">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route($dashboardRoute::name('users.destroy'), $editUser->id) }}" method="POST"
                            id="delete-user-form">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="btn btn-danger" style="width: 100%;"
                                onclick="event.preventDefault(); showDanger('Delete User', 'Are you sure you want to delete this user? This action cannot be undone.').then(confirmed => { if(confirmed) document.getElementById('delete-user-form').submit(); })">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                </svg>
            ...
            <form action="{{ route($dashboardRoute::name('users.suspend'), $editUser->id) }}" method="POST">
                ...
                <div class="modal-body">
                    <p style="margin-bottom: 1rem; color: var(--muted-foreground);">
                        You are about to suspend <strong>{{ $editUser->name }}</strong>. This will revoke all their active
                        sessions.
                    </p>
                    <div class="form-group">
    ...
    @push('scripts')
        <form id="delete-user-photo-form" action="{{ route($dashboardRoute::name('users.photo.delete'), $editUser->id) }}"
